...
class StarrySkyPainter extends CustomPainter {
  ...
  Path _createStarPath(Offset center, double size) {
    ...
    final double x0 = center.dx + halfSize * cos((rotation - halfStep) * degToRad);
    final double y0 = center.dy + halfSize * sin((rotation - halfStep) * degToRad);
    path.moveTo(x0, y0);

    for (int i = 1; i <= 5; i++) {
      final double x = center.dx + halfSize * cos((rotation + angleStep * i - halfStep) * degToRad);
      final double y = center.dy + halfSize * sin((rotation + angleStep * i - halfStep) * degToRad);
      path.lineTo(x, y);
    }
